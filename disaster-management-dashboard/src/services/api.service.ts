...
export class ApiService {
  ...
  checkBackendStatus(): Observable<{ codec: string, disaster: string }> {
    ...
    // The disaster service doesn't have a specific /health endpoint, so we check the /docs endpoint,
    // which is automatically generated by FastAPI and is a reliable indicator of service health.
    const disasterStatus$ = this.http.get(`${disasterBaseUrl}/docs`, { responseType: 'text' }).pipe(
      map(() => 'Online'),
      catchError(() => of('Offline'))
    );
