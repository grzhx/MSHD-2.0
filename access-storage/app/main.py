...
from .codec_router import router as codec_router

# Basic logging setup so every module shares the same formatter/level
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(name)s - %(message)s",
)
logger = logging.getLogger("disaster_backend")

# 在应用启动时根据 ORM 模型创建表（如果表不存在）
Base.metadata.create_all(bind=engine)

# 创建 FastAPI 实例（合并编码/字典与接入/存储能力）
app = FastAPI(title="多源灾情数据管理服务（模块1-4）", version="1.0.0")
# 挂载编码与字典服务路由
app.include_router(codec_router)

# 允许本地前端（Angular/Vite 默认 4200/5173）跨域访问
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
@app.get("/")
def root():
    return {"service": "disaster-backend", "status": "ok"}
...
def ingest_data(
    ...
    if mode == "encoded":
        ...
        logger.info("Ingest request (encoded) body=%s", body)
        ...
        logger.info("Ingest request (raw) body=%s", body)
...
def list_disaster_records(
    limit: int = Query(100, ge=1, le=1000, description="返回的最大条数"),
    offset: int = Query(0, ge=0, description="分页偏移量"),
    db: Session = Depends(get_db),
):
    """
    列表查询接口，供前端仪表盘和运维视图使用。
    默认按创建时间倒序返回，避免一次性拉取过多数据。
    """
    logger.info("List disaster records limit=%s offset=%s", limit, offset)
    records = (
        db.query(models.DisasterRecord)
        .order_by(models.DisasterRecord.created_at.desc())
        .offset(offset)
        .limit(limit)
        .all()
    )
...
def run_retention(db: Session = Depends(get_db)):
    ...
    logger.info("Retention job archived %s records (threshold=%s)", count, threshold.isoformat())
